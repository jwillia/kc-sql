...
public class HRManifestResource {
  ...
  public HRManifestResource() throws Exception {
    ...
    hrManifestSchema = sf.newSchema(new StreamSource(getClass().getResourceAsStream(SCHEMA_PATH)));
  ...
  public Response processManifest(@FormDataParam("file") InputStream uploadedInputStream) throws Exception {
    File tempFile = File.createTempFile("hrmanifest", Long.toString(new Date().getTime()));
